       ...
       # If we have the UID, continue
        if status == MIFAREReader.MI_OK:
            ...
            if email_exists.status_code == 200:
                ...
                update_id = requests.post("https://hakron.io/arcade/api/tags/" + str(hex(uid[0]))+"."+str(hex(uid[1]))+"."+str(hex(uid[2]))+"."+str(hex(uid[3])) + "/person", json={"person":email_exists.json()["_id"]})
                #if email does not exist
            else:
                print(format(str(hex(uid[0])), 'x')+"."+str(hex(uid[1]))+"."+str(hex(uid[2]))+"."+str(hex(uid[3])))
